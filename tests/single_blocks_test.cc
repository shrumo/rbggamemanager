...
void TestPlayerSwitch() {
  ...
  assert(game.current_player() ==
         game.declarations().players_resolver().Id("blue"));
}

void TestMultipleSearchStep() {
  auto game = CreateGameState(kSmallGameDeclarations +
                              "#rules = ->red (->blue + ->red)");
  auto moves = game.Moves();
  assert(moves.size() == 2);
}
...
void TestShiftCheckSearchStep() {
  auto game =
      CreateGameState(kSmallGameDeclarations + "#rules = ->red right ->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
  assert(moves[0][0].vertex == 2);
  auto game_no_move = CreateGameState(kSmallGameDeclarations +
...
void TestOnCheckSearchStep() {
  auto game =
      CreateGameState(kSmallGameDeclarations + "#rules = ->red {b} ->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
  auto game_no_move =
      CreateGameState(kSmallGameDeclarations + "#rules = ->red {e} ->blue");
  auto moves_empty = game_no_move.Moves();
  assert(moves_empty.empty());
}
...
void TestAssignment() {
  auto game =
      CreateGameState(kSmallGameDeclarations +
                      "#rules = ->red [$turn = 10] [$blue = 15] ->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
  assert(moves[0].size() == 3);
  game.Apply(moves[0]);
  assert(game.variables_values()[game.declarations().variables_resolver().Id(
             "turn")] == 10);
  assert(game.variables_values()[game.declarations().variables_resolver().Id(
             "blue")] == 15);
  auto game_no_move =
      CreateGameState(kSmallGameDeclarations +
                      "#rules = ->red [$turn = 10] [$blue = turn + 70] ->blue");
  auto moves_empty = game_no_move.Moves();
  assert(moves_empty.empty());
}
...
void TestLessComparison() {
  auto game = CreateGameState(
      kSmallGameDeclarations +
      "#rules = ->red [$blue = 6] [$turn = 7] {$ blue < turn + 2} ->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
...
void TestLessEqualComparison() {
  auto game = CreateGameState(
      kSmallGameDeclarations +
      "#rules = ->red [$blue = 6] [$turn = 7] {$ blue <= turn - 1} ->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
  auto game_no_move = CreateGameState(
      kSmallGameDeclarations +
...
void TestEqualComparison() {
  ...
  auto game_no_move = CreateGameState(
      kSmallGameDeclarations +
      "#rules = ->red [$blue = 6] [$turn = 7] {$ blue == turn} ->blue");
  auto moves_empty = game_no_move.Moves();
  assert(moves_empty.empty());
}
...
void TestPiecesCounts() {
  auto game = CreateGameState(
      kSmallGameDeclarations +
      "#rules = ->red {$ e == 2 } {$ r == 1} {$ b == 1} {$ r == b} ->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
}
...
void TestNotEqualComparison() {
  auto game = CreateGameState(
      kSmallGameDeclarations +
      "#rules = ->red [$blue = 6] [$turn = 7] {$ blue != turn - 7} ->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
  auto game_no_move = CreateGameState(
      kSmallGameDeclarations +
...
void TestConditionCheckStep() {
  auto game =
      CreateGameState(kSmallGameDeclarations +
                      "#rules = ->red {? right [e] (left + down)* {e} }->blue");
  auto moves = game.Moves();
  assert(moves.size() == 1);
  auto game_no_move = CreateGameState(
      kSmallGameDeclarations +
      "#rules = ->red  {? right [e] (left + down)* {e} up {r}} ->blue");
  auto moves_empty = game_no_move.Moves();
  assert(moves_empty.empty());
}
