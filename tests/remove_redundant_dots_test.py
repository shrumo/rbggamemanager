...
# assert('#rules' in out)
print(out)
print('Simple case with right redundant dots removed:')
print(out)
rules = out[out.find('#rules'):]
rules = rules.replace(' ', '').replace('\n', '')
assert(rules == "#rules=(->red.(up+down+left+right)*[r]{r}.(up+down+left+right)*right.(up+down)->blue)")
...
out = subprocess.getoutput(
    './remove_redundant_dots {} --state_to_erase right --modifiers_as_dots false'.format(tmp_game_filename))
# assert('#rules' in out)
print(out)
print('Simple case with right redundant dots removed, not treating modifiers as dots:')
print(out)
rules = out[out.find('#rules'):]
rules = rules.replace(' ', '').replace('\n', '')
assert(rules == "#rules=(->red.(up+down+left+right)*.[r]{r}(up+down+left+right)*right.(up+down)->blue)")
