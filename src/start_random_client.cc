...
int main(int argc, const char* argv[]) {
  ...
  if (args.positional_args.size() != 2) {
    std::cout << "Usage: " << argv[0]
              << " <host> <port> [--seed <random_seed>] [--sleep <sleep_time "
                 "ms>] [--time <time_to_disconnect ms>] "
                 "[--additional_preparation_time <preparation_time ms>]"
              << std::endl;
    return 0;
  }
  ...
  if (args.flags.find("additional_preparation_time") != args.flags.end()) {
    additional_compilation_time =
        std::stod(args.flags.at("additional_preparation_time")) / 1000.0;
  }
  ...
  std::cout << "I am player: "
            << state.declarations().players_resolver().Name(assigned_player)
            << " (" << assigned_player << ")\n";
  std::cout << "I had " << client.preparation_time()
            << " seconds to prepare to play" << std::endl;

  std::this_thread::sleep_for(
      std::chrono::duration<double>(additional_compilation_time));
  ...
  do {
    ...
    while (!moves.empty()) {
      if (state.current_player() == assigned_player) {
        std::cout << "Its my turn, waiting for deadline from server."
                  << std::endl;
        double deadline = client.ReadDeadline();
        std::cout << "The deadline to make the move is " << deadline
                  << " seconds." << std::endl;
        std::cout << "\n";
        std::cout << RectangularBoardDescription(state.board_content(),
                                                 state.declarations())
                  << std::endl;
        ...
        for (const auto& mod : move) {
          std::cout
              << "\t"
              << state.declarations().initial_board().vertices_names().Name(
                     mod.vertex)
              << " (" << mod.vertex << ") "
