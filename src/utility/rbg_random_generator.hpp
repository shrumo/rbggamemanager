 ...
 * 1 -- std::mt19937 with std::uniform_int_distribution (the standard unbiased
 * method) 2 -- Java's algorithm (lcg48, unbiased) 3 -- lcg48 with
 * multiplication method (biased) 4 -- xorShift64 with multiplication method
 * (biased)
 */
#include <cstdint>
//*********************************************************************
...
struct RBGRandomGenerator {
  ...
  unsigned int uniform_choice(const unsigned int upper_bound) {
    return std::uniform_int_distribution<unsigned int>(0, upper_bound - 1)(
        random_generator);
  }
};
//*********************************************************************
...
struct RBGRandomGenerator {
  ...
  RBGRandomGenerator(const uint64_t _seed)
      : seed((_seed ^ 0x5DEECE66DUL) & ((1UL << 48) - 1)) {}
