...
class GetMoveTypeFn : public AstFunction<MoveType> {
  public:
    MoveType ShiftCase(const rbg_parser::shift &) override { return MoveType::kShiftType; }

    MoveType OnCase(const rbg_parser::ons &) override { return MoveType::kOnType; }

    MoveType OffCase(const rbg_parser::off &) override { return MoveType::kOffType; }

    MoveType AssignmentCase(const rbg_parser::assignment &) override { return MoveType::kAssignmentType; }

    MoveType PlayerSwitchCase(const rbg_parser::player_switch &) override { return MoveType::kPlayerSwitchType; }

    MoveType KeeperSwitchCase(const rbg_parser::keeper_switch &) override { return MoveType::kKeeperSwitchType; }

    MoveType MoveCheckCase(const rbg_parser::move_check &) override { return MoveType::kConditionCheck; }

    MoveType ArithmeticComparisonCase(const rbg_parser::arithmetic_comparison &) override {
