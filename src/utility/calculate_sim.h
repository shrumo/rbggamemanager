...
namespace rbg {
  ...
  SimResult RandomSimulations(GameState &state, uint sim_count, unsigned long seed) {
    ...
    for (uint r = 0; r < players_count; r++) {
      ...
      player_variable_ids.push_back(state.declarations().variables_resolver().Id(player_name));
    }
    ...
    for (uint i = 0; i < sim_count; i++) {
      ...
      for (uint rvi = 0; rvi < player_variable_ids.size(); rvi++)
        result.sum_goals[rvi] += state.variables_values()[player_variable_ids[rvi]];
      state.Reset();
    }
    return result;
  }
