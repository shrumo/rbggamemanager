...
template <typename ResultType> class MoveFunction : private MoveVisitor {
public:
  virtual ResultType ShiftCase(const Shift &move) { return DefaultCase(move); }

  virtual ResultType ShiftTableCase(const ShiftTable &move) {
    return DefaultCase(move);
  }

  virtual ResultType
  ArithmeticComparisonCase(const ArithmeticComparison &move) {
    return DefaultCase(move);
  }
  ...
  virtual ResultType PlayerSwitchCase(const PlayerSwitch &move) {
    return DefaultCase(move);
  }

  virtual ResultType KeeperSwitchCase(const KeeperSwitch &move) {
    return DefaultCase(move);
  }

  virtual ResultType AssignmentCase(const Assignment &move) {
    return DefaultCase(move);
  }

  virtual ResultType PlayerCheckCase(const PlayerCheck &move) {
    return DefaultCase(move);
  }

  virtual ResultType ConditionCase(const Condition &move) {
    return DefaultCase(move);
  }

  virtual ResultType VisitedQueryCase(const VisitedQuery &move) {
    return DefaultCase(move);
  }

  virtual ResultType EmptyBackwardCase(const EmptyBackward &move) {
    return DefaultCase(move);
  }

  virtual ResultType EmptyForwardCase(const EmptyForward &move) {
    return DefaultCase(move);
  }
