...
namespace rbg {
  ...
  private:
    ...
    const std::list<edge_id_t>& edges() const {
      return edges_;
    }

    private:
      std::unordered_map<edge_id_t, std::list<edge_id_t>::iterator> edges_positions_;
  ...
  public:
    ...
    const EdgeContent& content() const {
      return content_;
    }

    EdgeContent& modifiable_content() {
      return content_;
    }
  ...
  public:
    ...
    Edge<EdgeContent> &GetEdge(edge_id_t edge_id) {
    ...
    decltype(auto) EdgesTo(node_t node_id) {
      ...
      return std_ext::range<
          std_ext::transform_iterator<
              ...
              decltype(extract_edge)>>
              (std_ext::transform_iterator<
                  typename std::unordered_set<edge_id_t>::const_iterator,decltype(extract_edge)
                  >(edges_ids_to(node_id).begin(), extract_edge),
              std_ext::transform_iterator<
                  typename std::unordered_set<edge_id_t>::const_iterator,decltype(extract_edge)
                  >(edges_ids_to(node_id).end(), extract_edge), edges_ids_to(node_id).size());
    }
    ...
    using NodesRange = std_ext::range<
        std_ext::transform_iterator<
    ...
    Node<EdgeContent> &GetNode(node_t node_id) {
...
std::string GraphDescription(const rbg::Graph<EdgeContent> &g, ContentPrinter printer) {
  ...
  for (const auto& node : nodes) {
    ...
    for (const auto& edge : edges) {
      ...
      result << edge.to() << " (edge id: " << edge.id() << ", content: " << printer(edge.content())
             << ")";
    }
  }
  return result.str();
