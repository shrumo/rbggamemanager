...
public:
  ResettableBitArray(uint size, bool value) : data_(size, (unsigned int)value) {
    threshold_ = 1;
  }
  ...
  bool operator[](const uint index) const { return data_[index] == threshold_; }

  void set(const uint index) { data_[index] = threshold_; }
  ...
  void resize(uint new_size) { data_.resize(new_size); }
...
public:
  ...
  void RevertToDepth(uint depth) { depth_ = depth; }
  ...
  uint bit_array_size() const { return bit_array_size_; }

  ResettableBitArray &top_array() { return bit_arrays_[depth_ - 1]; }
...
public:
  ResetableBitArrayStackChunk(ResettableBitArrayStack *parent, uint begin_index,
                              uint size)
