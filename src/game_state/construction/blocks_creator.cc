...
uint CreateStepsInCollection(
    const Nfa<std::unique_ptr<Move>> &nfa, BlocksCollection *collection,
...
class BlockCreator : public MoveFunction<BlocksCreatorResult<Branch>> {
 public:
  ...
  BlocksCreatorResult<Branch> ShiftCase(const Shift &move) override {
    auto block = CreateBlockUniquePtr(ShiftAction(move.edge_id()),
                                      OutOfBoundsTest{}, Branch{});
  ...
  BlocksCreatorResult<Branch> OffCase(const Off &move) override {
    auto block = CreateBlockUniquePtr(
        OffApplication(move.piece(), move.index()), Branch{});
    auto branch = block->content().branch();

    if (register_modifiers_) {
      collection_->RegisterModifier(move.index(),
                                    block->template GetAction<0>());
    }
  ...
  BlocksCreatorResult<Branch> PlayerSwitchCase(
    ...
    collection_->RegisterSwitch(move.index(),
                                block->template GetSubAbstractBlock<1>());
    uint step_index = collection_->AddBlock(std::move(block));
    return {step_index, branch};
  }
  ...
  BlocksCreatorResult<Branch> AssignmentCase(const Assignment &move) override {
    auto block = CreateBlockUniquePtr(
        AssignmentAction(move.get_variable(),
                         CreateArithmeticOperation(move.get_value_expression()),
                         move.index()),
        VariableBoundsTest(move.get_variable()), Branch{});
    auto branch = block->content().branch();
    if (register_modifiers_) {
      collection_->RegisterModifier(move.index(),
                                    block->template GetAction<0>());
...
class ConditionVisitedCheckCreator
 ...
 public:
  ...
  BlocksCreatorResult<Branch> VisitedQueryCase(const VisitedQuery &) override {
    auto block = CreateBlockUniquePtr(
        ConditionVisitedCheckTest(
            collection_->GetVisitedArrayChunk(visited_check_index_),
            collection_->GetConditionsResultsArrayChunk(
                condition_results_index_)),
        Branch{});
    auto branch = block->content().branch();
    uint step_index = collection_->AddBlock(std::move(block));
    return {step_index, branch};
...
class ConditionCheckCreator : public MoveFunction<BlocksCreatorResult<Branch>> {
 public:
  ...
  BlocksCreatorResult<Branch> ConditionCase(const Condition &move) override {
    if (move.negated()) {
      uint condition_initial_id = CreateStepsInCollection(
          move.nfa(), collection_, declarations_, visited_check_indices_, true);
      Block *condition_initial_block =
          collection_->GetBlockPointer(condition_initial_id);
      auto block = CreateBlockUniquePtr(
          NegatedConditionCheckTest(condition_initial_block), Branch{});
      auto branch = block->content().branch();
      uint step_index = collection_->AddBlock(std::move(block));
      return {step_index, branch};
    } else {
      ...
      Block *condition_initial_block =
          collection_->GetBlockPointer(condition_initial_id);
      auto block = CreateBlockUniquePtr(
          ConditionCheckTest(condition_initial_block), Branch{});
      auto branch = block->content().branch();
      uint step_index = collection_->AddBlock(std::move(block));
      return {step_index, branch};
    }
...
void VisitedCheckIndicesRecursive(
    const Graph<std::unique_ptr<Move>> &game_graph, node_t current,
    bool condition, std::unordered_map<edge_id_t, uint> *visited_check_indices,
  ...
  for (const auto &edge : game_graph.EdgesFrom(current)) {
    ...
    if (edge.content()->type() == MoveType::kConditionCheck) {
      const auto &condition_move =
          dynamic_cast<const Condition &>(*edge.content());
      VisitedCheckIndicesRecursive(
          condition_move.nfa().graph, condition_move.nfa().initial, true,
          visited_check_indices, conditions_visited_check_indices, visited);
    }
    if (visited->find(next) == visited->end()) {
      VisitedCheckIndicesRecursive(game_graph, next, condition,
                                   visited_check_indices,
                                   conditions_visited_check_indices, visited);
    }
  }
}

std::pair<std::unordered_map<edge_id_t, uint>,
          std::unordered_map<edge_id_t, uint>>
VisitedCheckIndices(const Nfa<std::unique_ptr<Move>> &nfa) {
  ...
  std::unordered_set<node_t> visited;
  VisitedCheckIndicesRecursive(nfa.graph, nfa.initial, false, &visited_indices,
                               &conditions_indices, &visited);
  return {visited_indices, conditions_indices};
}

void CreateStepsInCollectionRecursive(
    const Graph<std::unique_ptr<Move>> &game_graph,
    BlocksCollection *collection, const Declarations &declarations,
    const std::pair<std::unordered_map<edge_id_t, uint>,
                    std::unordered_map<edge_id_t, uint>> &visited_check_indices,
    node_t current, unordered_map<node_t, uint> *nodes_blocks,
    bool condition = false) {
  if (game_graph.EdgesFrom(current).size() == 0) {
    (*nodes_blocks)[current] =
        collection->AddBlock(CreateBlockUniquePtr(BranchEmpty()));
    ...
    if (edge.content()->type() == MoveType::kVisitedCheck) {
      ...
      if (condition) {
        uint condition_results_index =
            visited_check_indices.second.at(edge.id());
        result = ConditionVisitedCheckCreator<BranchSingle>(
            collection, declarations, visited_check_index,
            condition_results_index)(*edge.content());
      } else {
        result = VisitedCheckCreator<BranchSingle>(
            collection, declarations, visited_check_index)(*edge.content());
      }
    } else if (edge.content()->type() == MoveType::kConditionCheck) {
      result = ConditionCheckCreator<BranchSingle>(
          collection, declarations, game_graph,
          visited_check_indices)(*edge.content());
    } else {
      result = BlockCreator<BranchSingle>(collection, declarations,
                                          !condition)(*edge.content());
    }
    (*nodes_blocks)[current] = result.block_collection_index;
    if (nodes_blocks->find(next) == nodes_blocks->end()) {
      CreateStepsInCollectionRecursive(game_graph, collection, declarations,
                                       visited_check_indices, next,
                                       nodes_blocks, condition);
    }
    result.branch_pointer->AddNext(
        collection->GetBlockPointer(nodes_blocks->at(next)));
  } else {
    auto result = CreateMultipleBranch(collection);
    (*nodes_blocks)[current] = result.block_collection_index;

    for (const auto &edge : game_graph.EdgesFrom(current)) {
      assert((edge.content()->type() == MoveType::kBackwardEmpty ||
              edge.content()->type() == MoveType::kForwardEmpty) &&
...
uint CreateStepsInCollection(
  ...
  CreateStepsInCollectionRecursive(nfa.graph, collection, declarations,
                                   visited_check_indices, nfa.initial,
                                   &nodes_blocks, condition);
...
BlocksCollection rbg::CreateSearchSteps(
  ...
  uint initial_index = CreateStepsInCollection(game_nfa, &collection,
                                               declarations, visited_indices);
  collection.set_initial(initial_index);
  return collection;
}