...
CreateStepsInCollection(const Nfa<std::unique_ptr<Move>> &nfa, BlocksCollection &collection,
                        ...
                        const std::pair<std::unordered_map<edge_id_t, uint>, std::unordered_map<edge_id_t,uint>> &visited_check_indices, bool condition);
...
void VisitedCheckIndicesRecursive(const Graph<std::unique_ptr<Move>> &game_graph,
  ...
  for (const auto &edge : game_graph.EdgesFrom(current)) {
    ...
    if (visited->find(next) == visited->end()) {
      VisitedCheckIndicesRecursive(game_graph, next, condition, visited_check_indices, conditions_visited_check_indices,
                                   visited);
    }
  }
}
  ...
  if (game_graph.EdgesFrom(current).size() == 0) {
    ...
    if (edge.content()->type() == MoveType::kVisitedCheck) {
      ...
      if(condition) {
        ...
        result = ConditionVisitedCheckCreator<BranchSingle>(collection, declarations, visited_check_index, condition_results_index)(
            *edge.content());
      }
      else {
...
CreateStepsInCollection(const Nfa<std::unique_ptr<Move>> &nfa, BlocksCollection &collection,
                        const Declarations &declarations,
                        const std::pair<std::unordered_map<edge_id_t, uint>, std::unordered_map<edge_id_t,uint>> &visited_check_indices, bool condition = false) {
