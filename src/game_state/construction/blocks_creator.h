...
public:
  BlocksCollection(uint visited_checks_count, uint condition_checks_count,
      ...
      : visited_info_stack_(std::make_unique<ResettableBitArrayStack>(
            visited_checks_count * board_size)),
        conditions_results_(std::make_unique<ResettableBitArrayStack>(
            condition_checks_count * board_size)),
  ...
  ResetableBitArrayStackChunk GetConditionsResultsArrayChunk(uint index) {
    return {conditions_results_.get(),
            index * conditions_results_->bit_array_size() /
                condition_results_count_,
  ...
  void PushVisitedStack() { visited_info_stack_->Push(); }
