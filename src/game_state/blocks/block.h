...
public:
  ...
  void AddNext(Block *step) {
    assert(!step && "You cannot set next step to an empty branch.");
  }

  void RunNext(GameState *) {}

  bool RunNextAndApplyFirst(GameState *, std::vector<ActionRevertInfo> *,
                            vertex_id_t) {
    return false;
  }
...
public:
  ...
  void RunNext(GameState *state) { next_->Run(state); }
...
public:
  ...
  void Run(GameState *state) { branch_.RunNext(state); }
  ...
  branch_type *branch() { return &branch_; }
...
public:
  ...
  /* ------------------------ If the action is of type CHECK, those methods are
   * defined. -------------------------- */
  ...
  RunAndApplyFirst(GameState *state,
                   std::vector<ActionRevertInfo> *revert_infos,
                   vertex_id_t revert_vertex) {
    if (action_.Check(state))
      return next_elements_.RunAndApplyFirst(state, revert_infos,
                                             revert_vertex);
    return false;
  }
  ...
  /* ------------------------ If the action is of type APPLICATION, those
   * methods are defined. -------------------- */
  template <ActionTypeTrait ActionKind = Action::kind>
  ...
  /* ------------------------ If the action is of type MODIFIER, those methods
   * are defined. ----------------------- */

  template <ActionTypeTrait ActionKind = Action::kind>
  typename std::enable_if<ActionKind == ActionTypeTrait::kModifier, void>::type
  Run(GameState *state) {
  ...
  template <ActionTypeTrait ActionKind = Action::kind>
  typename std::enable_if<ActionKind == ActionTypeTrait::kModifier, bool>::type
  RunAndApplyFirst(GameState *state,
                   std::vector<ActionRevertInfo> *revert_infos,
                   vertex_id_t revert_vertex) {
    action_.PushVisitedStackAndAddToApplied(state);
    ...
    revert_infos->push_back(
        ActionRevertInfo{&action_, revert_vertex, action_revert_info});
    if (next_elements_.RunAndApplyFirst(state, revert_infos,
                                        action_.modified_position(state))) {
      return true;
    }
    action_.Revert(state, action_revert_info);
    action_.PopVisitedStackAndAddToApplied(state);
  ...
  /* ------------------------ If the action is of type SWITCH, those methods are
   * defined. ----------------------- */

  template <ActionTypeTrait ActionKind = Action::kind>
  typename std::enable_if<ActionKind == ActionTypeTrait::kSwitch, void>::type
  Run(GameState *state) {
    action_.PushVisitedStackAndAddToApplied(state);
    typename Action::revert_info_t action_revert_info = action_.Apply(state);
  ...
  template <ActionTypeTrait ActionKind = Action::kind>
  typename std::enable_if<ActionKind == ActionTypeTrait::kSwitch, bool>::type
  RunAndApplyFirst(GameState *state,
                   std::vector<ActionRevertInfo> *revert_infos, vertex_id_t) {
    action_.PushVisitedStackAndAddToApplied(state);
    typename Action::revert_info_t action_revert_info = action_.Apply(state);
    revert_infos->push_back(ActionRevertInfo{
        &action_, action_.modified_position(state), action_revert_info});
    return true;
  }
  ...
  /* ------------------------ If the action is of type SPECIAL, those methods
   * are defined. ----------------------- */
  /* This kind of action gives tha action full control of what is happening in
   * each call. */

  template <ActionTypeTrait ActionKind = Action::kind>
  typename std::enable_if<ActionKind == ActionTypeTrait::kSpecial, void>::type
  Run(GameState *state) {
    action_.Run(state, &next_elements_);
  }

  template <ActionTypeTrait ActionKind = Action::kind>
  typename std::enable_if<ActionKind == ActionTypeTrait::kSpecial, bool>::type
  RunAndApplyFirst(GameState *state,
                   std::vector<ActionRevertInfo> *revert_infos,
                   vertex_id_t revert_vertex) {
    return action_.RunAndApplyFirst(state, revert_infos, revert_vertex,
                                    &next_elements_);
  }
  ...
  /* ------------------------ Those functions are defined always in a
   * BlockContent -------------------------------- */
  Action *action() { return &action_; };

  const Action *action() const { return &action_; }

  branch_type *branch() { return next_elements_.branch(); }
...
struct ActionBlockContentGet<I, ActionsBlockContent<First, Rest...>> {
  using FirstElementType = typename ActionBlockContentGet<
      I - 1, ActionsBlockContent<Rest...>>::FirstElementType;
...
public:
  ...
  template <size_t I> auto GetSubAbstractBlock() {
    ...
    sub_pointers_.push_back(
        std::make_unique<ActionsBlockPointer<typename ActionBlockContentGet<
            I, ActionsBlockContent<BlockElements...>>::SubBlockType>>(
            sub_block_pointer));
  ...
  ActionsBlockContent<BlockElements...> &content() { return elements_; }
