...
public:
  ...
  MoveRevertInformation Apply(const std::vector<ModifierApplication> &move) {
    ...
    for (const auto &mod_application : move) {
      ...
      const ModifyingApplication *modifier =
          blocks_.Modifier(mod_application.modifier_index);
      current_pos_ = mod_application.vertex;
      revert_infos.push_back(
          ActionRevertInfo{modifier, previous_vertex, modifier->Apply(this)});
    }
  ...
  void Revert(const MoveRevertInformation &revert_info) {
    for (uint i = 0; i < revert_info.player_steps_revert_information.size();
         i++) {
      const auto &step =
          revert_info.player_steps_revert_information
  ...
  const BoardContent &board_content() const { return board_; }

  vertex_id_t current_pos() const { return current_pos_; }

  player_id_t current_player() const { return current_player_; }
  ...
  void Reset() {
    current_pos_ = 1;
    variables_values_ = std::vector<variable_value_t>(
        declarations_.variables_resolver().size(), 0);
...
private:
  ...
  MakeKeeperClosure(std::vector<ActionRevertInfo> &steps_revert_information) {
    ...
    while (exists && current_player_ == declarations_.keeper_id()) {
      applied_modifiers_.clear();
      moves_.clear();
      exists = current_block_->RunAndApplyFirst(this, &steps_revert_information,
                                                current_pos_);
      blocks_.ResetStacks();
      if (exists) {
        current_block_ =
            blocks_.Switch(applied_modifiers_.back().modifier_index);
        blocks_.ResetStacks();
        applied_modifiers_.clear();
      }
    }
