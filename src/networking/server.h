  ...
  void Run() {
    {
      ...
      while (clients_sockets_.size() !=
             state_.declarations().players_resolver().size() - 1) {
        tcp::socket socket(io_context_);
        std::cout << "Waiting for clients..."
                  << "("
                  << state_.declarations().players_resolver().size() - 1 -
                         clients_sockets_.size()
                  << " more needed)" << std::endl;
      ...
      for (uint i = 0; i < clients_sockets_.size(); i++) {
        std::cout << "Sending time for preparation which is "
                  << options_.time_for_player_preparation << " to client " << i
                  << std::endl;
        clients_sockets_[i].WriteString(
            std::to_string(options_.time_for_player_preparation));
        ...
        std::cout << "Sending player id "
                  << state_.declarations().players_resolver().Name(
                         client_player_id(i))
                  << "(" << client_player_id(i) << ")"
    ...
    if (!WaitForReady()) {
      for (uint i = 0; i < clients_sockets_.size(); i++) {
        if (players_ready_.find(i) == players_ready_.end()) {
          std::cout << "Client " << i << " was late with sending ready."
                    << std::endl;
    ...
    }
    {
      ...
      while (std::chrono::duration<double>(std::chrono::system_clock::now() -
                                           play_start)
                     .count() < options_.shutdown_after_seconds &&
        ...
        available_moves_ =
            std::unordered_set<GameMove>(moves.begin(), moves.end());
        uint available_moves_overall = available_moves_.size();
        bool exceeded_deadline = exceeded_initial_deadline;
        // Indexed py player socket index, gives out how many times this player
        // socket exceeded deadline when sending move
        ...
        while (!available_moves_.empty()) {
          ...
          auto manager_duration =
              std::chrono::duration<double>(manager_end - manager_begin)
                  .count();
          if (manager_duration > options_.deadline_seconds) {
            std::cout
                << "Manager exceeded the deadline. The calculation took him "
                << manager_duration << "s." << std::endl;
            exceeded_deadline = true;
            manager_deadline_exceeded = true;
          }
          clients_sockets_[player_socket_index(state_.current_player())]
              .WriteString(std::to_string(options_.deadline_seconds));
          auto move_string =
              clients_sockets_[player_socket_index(state_.current_player())]
                  .ReadString();
          auto end = std::chrono::system_clock::now();
          auto duration =
              std::chrono::duration<double>(end - manager_begin).count();

          if (duration > options_.deadline_seconds) {
            std::cout << "Deadline exceeded for client "
                      << player_socket_index(state_.current_player())
                      << " which is player "
                      << state_.declarations().players_resolver().Name(
                             state_.current_player())
                      << " (" << state_.current_player()
                      << ") the move was received after " << duration << "s."
                      << std::endl;
          ...
          if (available_moves_.find(move) == available_moves_.end()) {
            std::cout << "The move sent by the client is not legal"
                      << std::endl;
            ...
            for (const auto &mod : move) {
              std::cout << "\t"
                        << state_.declarations()
                               ...
                               .Name(mod.vertex)
                        << " (" << mod.vertex << ") "
                        << actions_translator_[mod.modifier_index] << " ("
        ...
        auto game_duration =
            std::chrono::duration<double>(game_end - game_begin).count();

        if (options_.results_logging_stream != nullptr) {
          auto &stream = *options_.results_logging_stream;
          stream << game_duration << " " << moves_done << " "
                 << available_moves_overall << " ";
          for (uint i = 0; i < clients_sockets_.size(); i++) {
            if (i > 0) {
              stream << " ";
            }
            auto player_name = state_.declarations().players_resolver().Name(
                client_player_id(i));
            variable_id_t player_variable_id =
                state_.declarations().variables_resolver().Id(player_name);
...
private:
  ...
  static uint player_socket_index(player_id_t player) { return player - 1; }
  ...
  bool WaitForReady() {
    {
      ...
      for (uint i = 0; i < clients_sockets_.size(); i++) {
        clients_sockets_[i].AsyncReadString(
            [this, i, time_sent](const asio::error_code &error,
                                 std::size_t size) {
  ...
  HandleReady(std::size_t message_size, uint player,
    ...
    std::string players_message =
        clients_sockets_[player].ExtractNextStringFromBuffer();
    if (players_message != "ready") {
      std::cout << "Player sent a bad ready message. She sent "
                << players_message << " instead of ready. Treating it as ready."
                << std::endl;
    }
    std::cout << "Got ready from player " << player << std::endl;
    if (std::chrono::duration<double>(ready_received_time - time_sent).count() <
        options_.time_for_player_preparation) {
