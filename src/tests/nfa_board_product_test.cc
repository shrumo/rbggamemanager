...
int main()
{
    ...
    NfaBoardProduct board_product(result, declarations.initial_board(), {/* vertex_id= */ 1});
    cout << "Nfa board product created starting with vertex: " << declarations.initial_board().vertices_names().Name(/* vertex_id= */ 1) << endl;
    node_t initial = board_product.node(/* vertex_id= */ 1, result.initial); // Corresponding to place A in initial vertex (left up)
    cout << "Initial node in NfaBoardGraph: " << initial << endl;
    cout << "Initial and final node in original graph: " << result.initial << " " << result.final << endl;

    assert(board_product.edges_ids_from(initial).size() == 1);
    assert(result.graph.GetEdge(board_product.GetEdge(board_product.edges_ids_from(initial).front()).content()).content()->type() == MoveType::kPlayerSwitchType);

    cout << "Original graph has " << result.graph.nodes().size() << " nodes." << endl;
    cout << "Board graph has " << board_product.nodes().size() << " nodes." << endl;
    ...
    while(!originals_to_visit.empty())
    {
        ...
        unordered_set<edge_id_t> original_outs(result.graph.edges_ids_from(node).begin(), result.graph.edges_ids_from(node).end());

        cout << "Original node " << node << endl;
        cout << "Original out edges: " << endl;
        for (auto &edge : result.graph.EdgesFrom(node))
        {
            cout << MoveDescription(*edge.content(), declarations) << " to " << edge.to() << endl;
        ...
        for (vertex_id_t vertex = 1; vertex < declarations.initial_board().vertices_count(); vertex++)
        {
            if (board_product.vertex_node_exists(vertex, node))
            {
                node_t board_product_vertex = board_product.node(vertex, node);
                cout << "\t" << board_product_vertex << " (" << declarations.initial_board().vertices_names().Name(vertex) << ", " << node << ")\n";
                cout << "\tWith outs:" << endl;
                for (auto &edge : board_product.EdgesFrom(board_product_vertex))
                {
                    ...
                    cout << "\t\t" << MoveDescription(*result.graph.GetEdge(edge.content()).content(), declarations) << " to " << edge.to() 
                    << " (" << declarations.initial_board().vertices_names().Name(vertex_node_to.first) << ", " << vertex_node_to.second << ") " << endl;
                }
            }
            else
            {
