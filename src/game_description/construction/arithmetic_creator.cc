...
class ArithmeticCreator
 ...
 public:
  ...
  unique_ptr<ArithmeticOperation> IntegerArithmeticCase(
      const rbg_parser::integer_arithmetic &expression) override {
    return make_unique<ConstantValue>(expression.get_content());
  }

  unique_ptr<ArithmeticOperation> VariableArithmeticCase(
      const rbg_parser::variable_arithmetic &expression) override {
    if (declarations_.pieces_resolver().contains(
            expression.get_content().to_string())) {
      return make_unique<PieceCountValue>(declarations_.pieces_resolver().Id(
          expression.get_content().to_string()));
    }
    return make_unique<VariableValue>(declarations_.variables_resolver().Id(
        expression.get_content().to_string()));
  }

  unique_ptr<ArithmeticOperation> ArithmeticOperationCase(
      const rbg_parser::arithmetic_operation &expression) override {
