...
Nfa<unique_ptr<Move>>
ThompsonsConstruction(const rbg_parser::game_move &rbg_move,
...
public:
  ...
  GraphCreatorResult
  ConcatenationCase(const rbg_parser::concatenation &concatenation) override {
    node_t initial = graph_.NewNode();
...
void EraseNoops(Nfa<unique_ptr<Move>> &nfa) {
  ...
  for (edge_id_t edge_id : edges_without_effect) {
    auto &transition = nfa.graph.GetEdge(edge_id);
