...
void EraseNoops(Nfa<unique_ptr<Move>> &nfa) {
  std::vector<node_t> nodes_without_effect = GetNoops(nfa);
  for (node_t node : nodes_without_effect) {
    auto &transition =
        nfa.graph.GetEdge(nfa.graph.edges_ids_from(node).front());
    if (node == nfa.initial) {
...
std::vector<node_t> rbg::GetNoops(const Nfa<std::unique_ptr<Move>> &nfa) {
  std::vector<node_t> noops;
  for (node_t node : std::vector<node_t>(nfa.graph.nodes().begin(),
                                         nfa.graph.nodes().end())) {
    const std::list<edge_id_t> &out_transitions =
        nfa.graph.edges_ids_from(node);
    if (out_transitions.size() == 1) {
      edge_id_t transition_id = out_transitions.front();
      auto &transition = nfa.graph.GetEdge(transition_id);
      if (transition.content()->type() == MoveType::kNoop) {
        noops.push_back(node);
      }
    }
  }
  return noops;
