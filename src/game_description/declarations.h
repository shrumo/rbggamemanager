 ...
 public:
  explicit Declarations(const rbg_parser::parsed_game &game)
    ...
    auto players_tokens = std::vector<std::pair<rbg_parser::token, uint>>(
        game.get_declarations().get_legal_players().begin(),
        game.get_declarations().get_legal_players().end());
    ...
    std::sort(players_tokens.begin(), players_tokens.end(),
              [](const std::pair<rbg_parser::token, uint> &a,
                 const std::pair<rbg_parser::token, uint> &b) {
    ...
    for (const auto &variable : game.get_declarations().get_legal_variables()) {
      auto variable_id =
          variables_resolver_.AddName(variable.first.to_string());
    ...
    for (uint id = 0; id < game.get_board().get_size(); id++) {
      vertex_id_t source_id = initial_board_.vertices_names().Id(
          game.get_board().get_vertex(id).to_string());
      for (const auto &edge : game.get_board().get_outgoing_edges(id)) {
        ...
        initial_board_.AddEdge(
            source_id, initial_board_.edges_names().Id(edge.first.to_string()),
            target_id);
      }
      initial_board_.initial_content().set(
          source_id, pieces_resolver_.AddName(
                         game.get_board().get_starting_piece(id).to_string()));
    }
  }

  const NamesResolver pieces_resolver() const { return pieces_resolver_; }

  const NamesResolver variables_resolver() const { return variables_resolver_; }

  const NamesResolver players_resolver() const { return players_resolver_; }

  variable_value_t variable_bound(variable_id_t variable) const {
    return variables_bounds_[variable];
  }

  const Board &initial_board() const { return initial_board_; }
