...
int main(int argc, const char *argv[]) {
  ...
  if (args.positional_args.size() != 2) {
    std::cout << "Usage: " << argv[0]
              << " <filename> <port> [--deadline <deadline_time ms>] "
                 "[--log_results <(result_logging_file|stdout)>] [--log_moves "
                 "<(moves_logging_file|stdout)> [--shutdown <shutdown_time "
                 "ms>] [--limit <games_limit>] [--time_for_player_preparation "
                 "<preparation_time ms>]"
              << std::endl;
    return 0;
  }
  ...
  options.port =
      static_cast<unsigned short>(std::stoi(args.positional_args[1]));
  ...
  if (args.flags.find("time_for_player_preparation") != args.flags.end()) {
    options.time_for_player_preparation =
        std::stod(args.flags.at("time_for_player_preparation")) / 1000.0;
  }
  ...
  if (args.flags.find("log_results") != args.flags.end()) {
    if (args.flags.at("log_results") != "stdout") {
      results_logging_file =
          std::make_unique<std::ofstream>(args.flags.at("log_results"));
  ...
  if (args.flags.find("log_moves") != args.flags.end()) {
    if (args.flags.at("log_moves") != "stdout") {
      moves_logging_file =
          std::make_unique<std::ofstream>(args.flags.at("log_moves"));
  ...
  if (args.flags.find("shutdown") != args.flags.end()) {
    options.shutdown_after_seconds =
        std::stod(args.flags.at("shutdown")) / 1000.0;
  }
